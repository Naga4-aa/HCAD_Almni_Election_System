...
const saveElection = async () => {
  ...
  try {
    ...
    const res = await api.put('admin/election/active/', payload)
    election.value = res.data
    electionMessage.value = 'Timeline saved.'
  } catch (err) {
...
<template>
  <div class="space-y-4">
    ...
    <div v-if="activeSection === 'timeline'" id="timeline" class="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm space-y-3">
      ...
      <div v-if="election" class="grid gap-3 sm:grid-cols-2">
        <div>
      ...
      <div class="flex flex-wrap gap-2 items-center">
        ...
        <div v-if="election" class="flex items-center gap-2 text-xs text-slate-600">
          <span>Status: {{ election.results_published ? 'Results published' : 'Not published' }}</span>
          <button
      ...
      <div class="bg-white rounded-2xl shadow-xl max-w-lg w-full p-6 space-y-3">
        ...
        <div class="grid gap-3 sm:grid-cols-2">
          ...
          <div>
            ...
            <input v-model="newVoter.campus_chapter" type="text" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" />
          </div>
          <div>
